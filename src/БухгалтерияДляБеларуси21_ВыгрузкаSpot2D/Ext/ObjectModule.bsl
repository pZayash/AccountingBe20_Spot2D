...
Процедура СоздатьСодержимоеФайлов() Экспорт 
	...
	запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ПериодВыгрузки.ДатаОкончания) - 24*3600); // Конец прошлого дня 
...
Функция ТекстЗапроса(пИмяФайла)
	...
	ИначеЕсли пИмяФайла = ИмяФайла_delivery() Тогда 
		...
		Возврат 
			...
			|	реализованныеТовары.Номенклатура.Код КАК Код_продукта_дистрибьютора,
			|	реализованныеТовары.Количество КАК Количество,
			|	реализованныеТовары.Сумма_отгрузки КАК Сумма_отгрузки,
			|	реализованныеТовары.Сумма_отгрузки_с_НДС КАК Сумма_отгрузки_с_НДС,
			|	"""" КАК Код_ТА,
