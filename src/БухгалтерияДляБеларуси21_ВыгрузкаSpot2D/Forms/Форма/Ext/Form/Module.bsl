...
&НаСервере
Процедура СоздатьСодержимоеФайловНаСервере(пКакРегламентноеЗадание = Ложь)
	
	обработкаОбъект = РеквизитФормыВЗначение("Объект");
	обработкаОбъект.СохранитьНастройки(Элементы.СписокНоменклатуры);
	
	Если пКакРегламентноеЗадание Тогда
		обработкаОбъект.ПериодВыгрузки_Установить();
...
&НаКлиенте
Процедура СоздатьСодержимоеФайловКакРегламентноеЗадание(Команда)
	какРегламентноеЗадание = Истина;
	СоздатьСодержимоеФайловНаСервере(какРегламентноеЗадание);
КонецПроцедуры
